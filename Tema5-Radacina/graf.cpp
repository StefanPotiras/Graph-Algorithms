...
void Graf::gasesteRadacina()
{
    ...
    int i=0,v;
     ...
     {
         QMessageBox messageBox;
         messageBox.critical(0, "NU", "GRAFUL NU ARE RADACINA!");
         messageBox.setFixedSize(800, 700);
         noduri.clear();
     }
     else
     {
         QMessageBox messageBox;
         messageBox.critical(0, "Da", "Radacina grafului este "+QString::number(root));
         qDebug()<<root;
         messageBox.setFixedSize(800, 700);
         noduri.clear();
     }
...
void Graf::DFS(Node x, std::vector<bool> &visited)
{
    ...
    for(auto& index:listaAdiacenta[x.getNumber()])
    {
        if(!visited[index.getNumber()])
            DFS(index,visited);
    }
